...
void ExitProcess()
{
	...
	for(int i = 0; i < size; ++i){
		if(machine->pageTable[i].valid==true)memoryManager->FreePage(machine->pageTable[i].physicalPage);}
...
ExceptionHandler(ExceptionType which)
{
	...
	if(memoryManager->IsAnyPageFree() == true){
		printf("id of current process %d\n",currentThread->id);
            	phyPageNo = memoryManager->AllocPage(currentThread->id,machine->pageTable[vpn]);
		printf("allocated physical page no %d\n",phyPageNo);
	}
		
        else
        {
            //will do this later
		phyPageNo = memoryManager->AllocByForce();
		printf("evicted physical page no %d\n",phyPageNo);
		firstEvict++;
		if(firstEvict==1)currentThread->space->init();
		currentThread->space->saveIntoSwapSpace(vpn);
        }
	syscallLock->Release();
	if(phyPageNo!=-1)currentThread->space->LoadIntoFreePage(addr, phyPageNo);
    	//else ExitProcess();
    } 
